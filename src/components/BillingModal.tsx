...
const PLAN_DETAILS = {
  Starter: { name: 'Starter Plan', price: 2000, desc: 'Up to 3 Operators, basic transaction logging' },
  Professional: { name: 'Professional Plan', price: 5000, desc: 'Up to 10 Operators, automated audit logging & daily reports' },
  Business: { name: 'Business Plan', price: 10000, desc: 'Unlimited Operators, priority network monitoring, maximum commissions' },
  Enterprise: { name: 'Enterprise Plan', price: 25000, desc: 'Full custom bank channels, dedicated support, and multi-branch audit logs' }
};
...
export function BillingModal({ isOpen, onClose, currentUser, showAppNotification, initialSelectedPlan }: BillingModalProps) {
  ...
  useEffect(() => {
    if (isOpen && activeTab === 'pay') {
      ...
      setSubmittedAmount(PLAN_DETAILS[selectedPlan].price.toString());
      setReceiptFile(null);
      setReceiptBase64('');
      setUploadError('');
    }
  }, [isOpen, activeTab, selectedPlan]);
  ...
  const handleConfirmPayment = async () => {
    ...
    // Prevent duplicate submission if active or pending review
    const hasPendingOrActive = mySubscription?.status === 'Active' || mySubscription?.status === 'Pending Review' || paymentHistory.some(p => p.status === 'Pending Review' || p.status === 'Pending');
    ...
    try {
      const planCost = PLAN_DETAILS[selectedPlan].price;
      const actualAmount = parseFloat(submittedAmount);

      if (isNaN(actualAmount) || actualAmount !== planCost) {
        const errorMsg = `Exact Payment Required: You entered ₦${submittedAmount || '0'}, but the ${PLAN_DETAILS[selectedPlan].name} costs ₦${planCost.toLocaleString()}. Please upload the exact payment receipt.`;
        setUploadError(errorMsg);
      ...
      const newPayment: ManualPaymentRequest & { userId: string; managerPhone: string; managerName: string } = {
        id: payId,
        ownerId: ownerId,
  ...
  const handleApproveSubscription = async (paymentOrRef?: any) => {
    ...
    try {
      ...
      const expectedPrice = PLAN_DETAILS[targetPlan as keyof typeof PLAN_DETAILS]?.price || 0;

      // STRICT SAFEGUARD: Block approval if amount doesn't match plan price
      if (amount !== expectedPrice && currentUser?.phone !== '08141106560') {
        const errorMsg = `Approval Denied: Exact Payment Required. Payment amount (₦${amount.toLocaleString()}) does not match ${targetPlan} price (₦${expectedPrice.toLocaleString()}).`;
        if (showAppNotification) showAppNotification(errorMsg, 'error');
        alert(errorMsg);
        ...
        <div className="px-6 py-4 border-b border-neutral-200 bg-white flex justify-between items-center shrink-0">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-[11px] text-neutral-500 font-medium">Verify or renew manual bank transfer subscription plans</p>
            </div>
          </div>
          <button 
            onClick={onClose}
          ...
          <AnimatePresence mode="wait">
            ...
            {activeTab === 'status' && (
              ...
              >
                ...
                {/* Subscriptions Status Cards */}
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  {/* Status Indicator Card */}
                  <div className="bg-white border border-neutral-200 p-5 rounded-2xl shadow-xs space-y-4">
                    <span className="text-[10px] font-bold text-neutral-400 uppercase tracking-widest font-mono block">Current Plan Status</span>
                    
                    <div className="flex items-center gap-3">
                      <div className="w-12 h-12 bg-neutral-50 border border-neutral-100 rounded-full flex items-center justify-center shrink-0">
                        <ShieldCheck className="w-6 h-6 text-[#00B87A]" />
                      </div>
                      <div>
                        <h3 className="text-lg font-black text-neutral-800 font-mono">{mySubscription?.plan || 'Free Trial'}</h3>
                    ...
                    <div className="border-t border-neutral-100 pt-3.5 space-y-2">
                      <div className="flex justify-between text-xs">
                        <span className="text-neutral-500 font-medium">Subscription Type:</span>
                        <span className="font-bold text-neutral-800">{mySubscription?.plan}</span>
                      </div>
                      {mySubscription?.subscriptionStartDate && (
                        <div className="flex justify-between text-xs">
                          <span className="text-neutral-500 font-medium">Start Date:</span>
                          <span className="font-mono text-neutral-800">{new Date(mySubscription.subscriptionStartDate).toLocaleDateString()}</span>
                        </div>
                      )}
                      {mySubscription?.subscriptionEndDate && (
                        <div className="flex justify-between text-xs">
                          <span className="text-neutral-500 font-medium">End Date:</span>
                          <span className="font-mono text-neutral-800">{new Date(mySubscription.subscriptionEndDate).toLocaleDateString()}</span>
                        </div>
                      )}
                      {mySubscription?.status === 'Trial' && mySubscription.trialEndDate && (
                        <div className="flex justify-between text-xs">
                          <span className="text-neutral-500 font-medium">Trial End Date:</span>
                          <span className="font-mono text-neutral-800">{new Date(mySubscription.trialEndDate).toLocaleDateString()}</span>
                        </div>
                      )}
                    </div>
                  </div>

                  {/* Pending/Rejected Status Alert Box */}
                  <div className="bg-white border border-neutral-200 p-5 rounded-2xl shadow-xs flex flex-col justify-between gap-4">
                    <span className="text-[10px] font-bold text-neutral-400 uppercase tracking-widest font-mono block">Verification Status Alert</span>
                    
                    {mySubscription?.status === 'Pending Review' ? (
                      <div className="bg-yellow-50/50 border border-yellow-100 p-4 rounded-xl flex items-start justify-between gap-3 flex-1">
                        <div className="flex items-start gap-3">
                          <Clock className="w-5 h-5 text-amber-500 shrink-0 mt-0.5 animate-spin" />
                          <div className="space-y-1">
                            <h4 className="font-bold text-xs text-amber-800">Payment Verification Pending</h4>
                            <p className="text-[11px] text-amber-700 leading-relaxed">
                              Your manual bank transfer reference <strong>{mySubscription?.lastPaymentReference}</strong> is currently undergoing manual review. Premium access will be enabled immediately after approval.
                            </p>
                          </div>
                        </div>
                        {(isSuperAdmin || currentUser?.phone === '08141106560' || (currentUser as any)?.phoneNumber === '08141106560' || currentUser?.id === '08141106560') && (
                          <button
                            ...
                            className="shrink-0 px-3.5 py-2 bg-[#00B87A] hover:bg-[#00a36c] text-white rounded-xl font-extrabold text-xs transition cursor-pointer flex items-center gap-1.5 shadow-md hover:shadow-lg transform active:scale-95"
                          >
                            <CheckCircle2 className="w-3.5 h-3.5" /> APPROVE SUBSCRIPTION
                          </button>
                        )}
                      </div>
                    ) : mySubscription?.status === 'Rejected' ? (
                      <div className="bg-rose-50/50 border border-rose-100 p-4 rounded-xl flex items-start gap-3 flex-1 flex-col justify-between">
                        <div className="flex items-start gap-3">
                          <XCircle className="w-5 h-5 text-red-500 shrink-0 mt-0.5" />
                          <div className="space-y-1">
                            <h4 className="font-bold text-xs text-red-800">Verification Rejected</h4>
                            <p className="text-[11px] text-red-700 leading-relaxed">
                              Your last manual transfer submission was rejected: <br />
                              <span className="font-mono bg-white/60 p-1 px-1.5 rounded block text-[10px] border border-rose-100 mt-1 font-bold">
                                "{mySubscription?.rejectionReason || 'No reason specified'}"
                              </span>
                            </p>
                          </div>
                        </div>
                        <button
                          onClick={() => setActiveTab('pay')}
                          className="mt-2 text-xs bg-red-600 hover:bg-red-700 text-white font-bold py-1.5 px-3 rounded-lg transition"
                        >
                          Upload New Receipt
                        </button>
                      </div>
                    ) : (
                      <div className="bg-emerald-50/50 border border-emerald-100 p-4 rounded-xl flex items-start gap-3 flex-1">
                        <CheckCircle2 className="w-5 h-5 text-emerald-500 shrink-0 mt-0.5" />
                        <div className="space-y-1">
                          <h4 className="font-bold text-xs text-emerald-800">Account Healthy</h4>
                          <p className="text-[11px] text-emerald-700 leading-relaxed">
                            No verification issues found. To secure premium support, and increase cashier capacity, you can upgrade your package at any time in the plans tab.
                          </p>
                        </div>
                      </div>
                    )}
                ...
                <div className="bg-white border border-neutral-200 rounded-2xl shadow-xs p-5">
                  <span className="text-[10px] font-bold text-neutral-400 uppercase tracking-widest font-mono block mb-3.5">System Notifications</span>
                  {notifications.length === 0 ? (
                    <p className="text-xs text-neutral-400 text-center py-6">No billing-related notifications yet.</p>
                  ) : (
                      ...
                      {notifications.map((n) => (
                        ...
                        >
                          <div className="flex gap-2.5">
                            <Bell className={`w-4 h-4 shrink-0 mt-0.5 ${n.read ? 'text-neutral-400' : 'text-[#00B87A] animate-swing'}`} />
              ...
              >
                ...
                <div className="space-y-3">
                  <label className="text-xs font-extrabold text-neutral-400 uppercase tracking-widest font-mono block">
                    {initialSelectedPlan ? 'Step 1: Your Selected Plan' : 'Step 1: Choose Your Subscription Plan'}
                  </label>
                  <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-3">
                    {(Object.keys(PLAN_DETAILS) as Array<keyof typeof PLAN_DETAILS>)
                      ...
                      .map((pKey) => {
                        ...
                        return (
                          <button
                            ...
                            className={`p-4 rounded-2xl border text-left flex flex-col justify-between transition cursor-pointer group ${
                              isSelected 
                                ? 'border-[#00B87A] bg-emerald-50/15 shadow-sm ring-1 ring-[#00B87A]' 
                                : 'border-neutral-200 hover:border-neutral-300 bg-white'
                            }`}
                          >
                            <div className="space-y-1">
                              <span className={`text-[10px] font-black uppercase font-mono tracking-wider px-2 py-0.5 rounded-full ${
                                isSelected ? 'bg-emerald-100 text-emerald-800' : 'bg-neutral-100 text-neutral-500'
                              }`}>{pKey}</span>
                              <span className="font-extrabold text-xs block text-neutral-800 group-hover:text-[#00B87A] pt-1">{p.name}</span>
                            </div>
                            <div className="pt-4 mt-auto">
                              <span className="text-sm font-black font-mono text-neutral-800">₦{p.price.toLocaleString()}</span>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  {/* Step 2: Transfer Bank Details Card */}
                  <div className="bg-white border border-neutral-200 p-5 rounded-2xl shadow-xs space-y-4">
                    <span className="text-xs font-extrabold text-neutral-400 uppercase tracking-widest font-mono block">Step 2: Transfer the subscription amount</span>
                    ...
                    <div className="space-y-3.5 bg-neutral-50 p-4 rounded-xl border border-neutral-100">
                      <div>
                        ...
                        <span className="text-sm font-black text-neutral-800">Access Bank</span>
                      </div>
                      <div>
                      ...
                      <div className="flex justify-between items-center">
                        <div>
                          ...
                          <span className="text-base font-extrabold text-[#00B87A] font-mono tracking-wider">1515915137</span>
                        </div>
                        <button
                          onClick={() => copyToClipboard('1515915137', 'account')}
                          ...
                          <div>
                            ...
                            <div className="relative mt-1">
                              ...
                              <input
                                ...
                                placeholder={PLAN_DETAILS[selectedPlan].price.toString()}
                            ...
                            <p className="text-[9px] text-amber-600 font-bold mt-1 uppercase">
                              * EXACTLY ₦{PLAN_DETAILS[selectedPlan].price.toLocaleString()} for {PLAN_DETAILS[selectedPlan].name}
                            </p>
                          </div>
                        </div>
                    ...
                    <div className="space-y-1.5 p-3.5 bg-amber-50/40 border border-amber-100 rounded-xl text-[11px] text-amber-800 leading-relaxed">
                      ...
                      <ul className="list-decimal pl-4.5 space-y-1 font-medium text-amber-700">
                        <li>Transfer <strong>₦{PLAN_DETAILS[selectedPlan].price.toLocaleString()}</strong> to the Access Bank account above.</li>
                  ...
                  {/* Step 3: Receipt Upload Form Card */}
                  <div className="bg-white border border-neutral-200 p-5 rounded-2xl shadow-xs flex flex-col justify-between">
                    <div>
                      <span className="text-xs font-extrabold text-neutral-400 uppercase tracking-widest font-mono block mb-4">Step 3: Upload receipt and confirm</span>
                      ...
                      <div className="pt-4 mt-6 border-t border-neutral-100">
                        ...
                        >
                          {isSubmittingPayment ? (
                            ...
                            <>
                              <ShieldCheck className="w-4 h-4" />
                              Confirm Payment (₦{PLAN_DETAILS[selectedPlan].price.toLocaleString()})
                            </>
                          )}
                        </button>
                      </div>
                      ...
                      {paymentHistory.length === 0 ? (
                        ...
                        paymentHistory.map((h) => (
                          <tr key={h.id} className="hover:bg-neutral-50/50 transition">
                            <td className="p-4 font-extrabold text-neutral-800">{h.plan} Plan</td>
                            ...
                            <td className="p-4">
                              <div className="flex items-center gap-2">
                                ...
                                {(h.status === 'Pending Review' || h.status === 'Pending') && (
                                  ...
                                  (() => {
                                   const expected = PLAN_DETAILS[h.plan as keyof typeof PLAN_DETAILS]?.price || 0;
                                   const isMismatched = h.amount !== expected;
                      ...
                      {filteredAdminPayments.length === 0 ? (
                        ...
                        filteredAdminPayments.map((p) => (
                          <tr key={p.id} className="hover:bg-neutral-50/50 transition">
                            ...
                            <td className="p-4 font-bold text-neutral-800">{p.plan}</td>

...
export function UnpaidChargesLedger({
  ...
  const [settleFeeMethod, setSettleFeeMethod] = useState<'Cash' | 'CardDebit' | 'Paystack'>('Cash');
  ...
  const handleReverseSettlement = async (tx: Transaction) => {
    ...
    if (tx.approvalStatus === 'approved' || tx.approved) {
      alert('Approved debts and unpaid charges cannot be reversed once finalized by the manager.');
      return;
    }
                  ...
                  return (
                    ...
                    >
                      ...
                      <div className="shrink-0 flex sm:flex-col items-end gap-1 text-right w-full sm:w-auto border-t sm:border-t-0 pt-2 sm:pt-0 border-neutral-100 justify-between">
                        ...
                        <div className="mt-1 inline-flex items-center gap-1 bg-emerald-50 text-emerald-700 font-bold text-[9px] px-2.5 py-1 rounded-lg uppercase tracking-wider font-mono border border-emerald-200 shadow-2xs">
          ...
          <div className="bg-white rounded-3xl max-w-md w-full max-h-[90vh] overflow-y-auto p-6 shadow-2xl space-y-5 relative border border-neutral-100 animate-in slide-in-from-bottom-4 duration-250">
            ...
            <div className="space-y-4">
              ...
              <div className="space-y-1.5">
                ...
                <div className="grid grid-cols-3 gap-2">
                  ...
                  <button
                    type="button"
                    onClick={() => setSettleFeeMethod('Paystack')}
                    className={`py-2 px-1 rounded-xl text-[10px] font-bold border transition cursor-pointer text-center uppercase font-mono ${
                      settleFeeMethod === 'Paystack'
                        ? 'bg-neutral-800 border-neutral-800 text-white font-black'
                        : 'bg-white border-neutral-200 text-neutral-500 hover:text-neutral-700'
                    }`}
                  >
            ...
            <div className="grid grid-cols-2 gap-2.5 pt-1">
              ...
              <button
                ...
                onClick={async () => {
                  ...
                  if (settleFeeMethod === 'Paystack') {
                    ...
                    try {
                      ...
                      if (!scriptLoaded) {
                        alert("❌ Failed to load Paystack payment library. Check your internet connection.");
                      ...
                      const paystackPop = (window as any).PaystackPop.setup({
                            ...
                            try {
                              ...
                              if (verifyData.status && verifyData.data?.status === "success") {
                                ...
                                const verifiedRecord = {
                                  ...newPaymentRecord,
                                  note: `${settlePaymentNote.trim() || 'Settle Payment'} (Paid via Paystack, Ref: ${response.reference})`
                                };
                                
                                const updatedVerifiedPayments = [...(settlingTx.chargePayments || []), verifiedRecord];
                                ...
                                if (currentUser?.role === 'Employee') {
                                  ...
                                  alert(`📢 Secure payment verified! Settlement request of ₦${currentPayment.toLocaleString()} submitted for approval.`);
                                } else {
                                  onUpdateTransaction({
                                    ...settlingTx,
                                    customerFee: finalCustomerFee,
                                  ...
                                  alert(`✅ Successfully processed secure Paystack payment of ₦${currentPayment.toLocaleString()}!`);
                                }
                                setSettlingTx(null);
                              } else {
                                alert("❌ Payment confirmation failed on verification.");
                              }
                            } catch (vErr) {
                        ...
                        onClose: () => {
                          alert("ℹ️ Paystack checkout was closed.");
                          setIsPaystackProcessing(false);
                        }
                      });
                      ...
                      alert("❌ Error launching secure payment popup.");
                      setIsPaystackProcessing(false);
                    }
                    return;
                  }
                  ...
                  if (currentUser?.role === 'Employee') {
                    ...
                    alert(`📢 Recorded settlement payment of ₦${currentPayment.toLocaleString()} for ${settlingTx.customerName || 'Customer'}! Pending Manager approval.`);
                    setSettlingTx(null);
                    return;
                  }
                ...
                className="w-full py-2.5 px-4 bg-[#00B87A] hover:bg-emerald-600 text-white font-black rounded-xl text-xs transition cursor-pointer flex items-center justify-center gap-1 font-mono uppercase shadow-md active:scale-95 disabled:opacity-50 disabled:cursor-not-allowed"
                disabled={isPaystackProcessing || (parseFloat(settleAmountPaid) || 0) <= 0}
              >

...
function appReducer(state: AppState, action: AppAction): AppState {
  ...
  switch (action.type) {
    ...
    case 'SET_INVENTORY_SALES': {
      nextState = {
        ...state,
        inventorySales: action.payload
      };
      break;
    }
    ...
    case 'UPDATE_INVENTORY_ITEM': {
      nextState = {
        ...state,
        inventoryItems: (state.inventoryItems || []).map(i => i.id === action.payload.id ? action.payload : i)
      };
      break;
    }
    case 'DELETE_INVENTORY_ITEM': {
      nextState = {
        ...state,
        inventoryItems: (state.inventoryItems || []).filter(i => i.id !== action.payload)
      };
      break;
    }
    case 'ADD_INVENTORY_SALE': {
      nextState = {
        ...state,
        inventorySales: [action.payload, ...(state.inventorySales || [])]
      };
      break;
    }
    case 'SET_SUPPLIERS': {
...
export default function App() {
  ...
  const [isStandalone, setIsStandalone] = useState(() => {
    ...
    return Capacitor.isNativePlatform() || window.matchMedia('(display-mode: standalone)').matches || (window.navigator as any).standalone === true;
  });
  ...
  // Auto-prompt PWA installation logic disabled
  // useEffect(() => {
  //   const checkAndPromptInstall = () => {
  ...
  //     }
  //
  //     const dismissCount = parseInt(localStorage.getItem('pwa_prompt_dismiss_count') || '0', 10);
  //     const lastDismissTime = parseInt(localStorage.getItem('pwa_prompt_last_dismiss') || '0', 10);
  //     const now = Date.now();
  //     
  //     // If never dismissed, or dismissed less than 3 times and it's been at least 12 hours since last dismiss
  //     if (dismissCount === 0 || (dismissCount < 3 && (now - lastDismissTime > 12 * 60 * 60 * 1000))) {
  //       // Delay popup slightly so it doesn't block immediate initial render
  ...
  const handleDismissPwaModal = () => {
    ...
    const count = parseInt(localStorage.getItem('pwa_prompt_dismiss_count') || '0', 10);
    localStorage.setItem('pwa_prompt_dismiss_count', (count + 1).toString());
    localStorage.setItem('pwa_prompt_last_dismiss', Date.now().toString());
  };

  const triggerDirectInstall = async () => {
    if (!deferredPrompt) {
      showAppNotification("To install: Tap browser menu ⋮ or Safari Share icon -> 'Add to Home Screen'.", "info");
      setIsPwaInstallModalOpen(false);
      return;
    }
    try {
      ...
      console.log(`[PWA] Direct prompt outcome: ${outcome}`);
      setDeferredPrompt(null);
      setIsInstallable(false);
      setIsPwaInstallModalOpen(false);
    } catch (err) {
      console.error('[PWA] Failed to prompt direct install:', err);
    }
  };
  ...
  // Load state from localStorage asynchronously
  useEffect(() => {
    const loadState = () => {
      try {
        ...
        if (saved) {
          ...
          dispatch({ type: 'SET_TRANSACTIONS', payload: Array.isArray(parsed.transactions) ? parsed.transactions : [] });
          // ... dispatch other state updates
          console.log('[initAppState] State loaded asynchronously.');
        }
      } catch (err) {
        console.error('[initAppState] Error during async state recovery:', err);
      }
    };
    loadState();
  }, []);
...
const getStoredApprovedTxIds = (): Set<string> => {
  ...
  const optimisticApprovedTxsRef = useRef<Set<string>>(null as any);
  if (optimisticApprovedTxsRef.current === null) {
    optimisticApprovedTxsRef.current = getStoredApprovedTxIds();
  }
  ...
  useEffect(() => {
    const handleNotificationEvent = (e: Event) => {
      ...
      if (customEvent.detail && customEvent.detail.message) {
        showAppNotification(customEvent.detail.message, customEvent.detail.type || 'info');
      }
    };
    window.addEventListener('app-show-notification', handleNotificationEvent);
    return () => {
      window.removeEventListener('app-show-notification', handleNotificationEvent);
    };
  }, []);
  ...
  const totalUnpaidDebtAmount = useMemo(() => {
    return unpaidTransactions.reduce((sum, tx) => sum + (tx.unpaidFeeAmount ?? tx.customerFee ?? 0), 0);
  }, [unpaidTransactions]);

  const [activeUnpaidIndex, setActiveUnpaidIndex] = useState(0);
  const [isHoveringUnpaidBanner, setIsHoveringUnpaidBanner] = useState(false);

  useEffect(() => {
    if (unpaidTransactions.length <= 1 || isHoveringUnpaidBanner) return;
    const interval = setInterval(() => {
      setActiveUnpaidIndex((prev) => (prev + 1) % unpaidTransactions.length);
    }, 4000); // rotate continuously every 4 seconds
    return () => clearInterval(interval);
  }, [unpaidTransactions.length, isHoveringUnpaidBanner]);
  ...
  // Background interval polling for subscription status every 5 minutes
  useEffect(() => {
    const ownerId = state.currentUser?.role === 'Manager' ? state.currentUser.id : state.currentUser?.ownerId;
    ...
    const pollSubscription = async () => {
      try {
        const res = await fetch(`/api/subscription/status/${ownerId}`);
        if (res.ok) {
          const result = await res.json();
          if (result.status === 'success' && result.data) {
            setActiveSubscription(result.data);
            console.log('[Subscription Poll] Background check successfully synced local state:', result.data);
          }
        }
      } catch (err) {
        console.warn('[Subscription Poll] Background status check failed:', err);
      }
    };

    // Run once initially, then every 5 minutes
    pollSubscription();
    const interval = setInterval(pollSubscription, 5 * 60 * 1000);
  ...
  const isPremiumLocked = useMemo(() => {
    ...
    if (activeSubscription.status === 'Active') {
      if (activeSubscription.subscriptionEndDate) {
        const isExpired = new Date(activeSubscription.subscriptionEndDate).getTime() < Date.now();
        if (isExpired) {
          return true; // The moment it reaches the time to finish, the account stops working!
        }
      }
      return false;
    }
  ...
  const [carouselSettleMethod, setCarouselSettleMethod] = useState<'Cash' | 'CardDebit' | 'Paystack'>('Cash');
  ...
  useEffect(() => {
    ...
    const invQuery = query(collection(db, 'inventory_items'), where('syncOwnerId', '==', syncOwnerId));
    const unsubscribeInvItems = onSnapshot(invQuery, (snapshot) => {
      const itemList: InventoryItem[] = [];
      snapshot.forEach((docSnap) => {
        const data = docSnap.data() as any;
        itemList.push({ id: docSnap.id, ...data } as InventoryItem);
      });
      itemList.sort((a, b) => (a.name || '').localeCompare(b.name || ''));
      dispatch({ type: 'SET_INVENTORY_ITEMS', payload: itemList });
    }, (err) => {
      console.warn('[Firestore] Inventory items sync error:', err);
    });

    // Subscribe to Inventory Sales
    const salesQuery = query(collection(db, 'inventory_sales'), where('syncOwnerId', '==', syncOwnerId));
    const unsubscribeInvSales = onSnapshot(salesQuery, (snapshot) => {
      const salesList: InventorySale[] = [];
      snapshot.forEach((docSnap) => {
        const data = docSnap.data() as any;
        salesList.push({ id: docSnap.id, ...data } as InventorySale);
      });
      salesList.sort((a, b) => new Date(b.createdAt).getTime() - new Date(a.createdAt).getTime());
      dispatch({ type: 'SET_INVENTORY_SALES', payload: salesList });
    }, (err) => {
      console.warn('[Firestore] Inventory sales sync error:', err);
    });

    // Subscribe to Suppliers
    const suppliersQuery = query(collection(db, 'suppliers'), where('syncOwnerId', '==', syncOwnerId));
    const unsubscribeSuppliers = onSnapshot(suppliersQuery, (snapshot) => {
      const supplierList: Supplier[] = [];
      snapshot.forEach((docSnap) => {
        const data = docSnap.data() as any;
        supplierList.push({ id: docSnap.id, ...data } as Supplier);
      });
      supplierList.sort((a, b) => (a.name || '').localeCompare(b.name || ''));
      dispatch({ type: 'SET_SUPPLIERS', payload: supplierList });
    }, (err) => {
      console.warn('[Firestore] Suppliers sync error:', err);
    });
  ...
  useEffect(() => {
    if (isLoading || !syncOwnerId || state.currentUser?.role !== 'Manager') return;

    const securityQuery = query(
      collection(db, 'security_events'),
      where('syncOwnerId', '==', syncOwnerId),
      orderBy('timestamp', 'desc'),
      limit(100)
    );

    const unsubscribeSecurity = onSnapshot(securityQuery, (snapshot) => {
      const eventList: SecurityEvent[] = [];
      snapshot.forEach((docSnap) => {
        const data = docSnap.data() as any;
        eventList.push({ id: docSnap.id, ...data } as SecurityEvent);
      });
      dispatch({ type: 'SET_SECURITY_EVENTS', payload: eventList });
    }, (err) => {
      console.warn('[Firestore] Security events sync error:', err);
    });

    return () => unsubscribeSecurity();
  }, [isLoading, syncOwnerId, state.currentUser?.role]);

  const handleLogSecurityEvent = async (type: SecurityEventType, description: string, metadata?: any) => {
    if (!state.currentUser) return;
    ...
    const newEvent: SecurityEvent = {
      ...
      userName: state.currentUser.name || state.currentUser.email || 'Unknown User',
      timestamp: new Date().toISOString(),
      syncOwnerId: syncOwnerId || undefined,
      metadata
    };

    dispatch({ type: 'ADD_SECURITY_EVENT', payload: newEvent });

    if (syncOwnerId) {
      try {
        const cleanData = prepareFirestoreData(newEvent, 'security_events');
        await setDoc(doc(db, 'security_events', eventId), cleanData);
      } catch (err) {
        handleFirestoreError(err, OperationType.WRITE, `security_events/${eventId}`);
      }
    }
  };
  ...
  const handleAddPosTerminal = async (term: PosTerminal) => {
    if (state.currentUser.role !== 'Manager') {
      showAppNotification('Permission denied: Only Store Managers have permission to register physical POS terminals.', 'error');
      return;
    }

    const activePlan = activeSubscription?.plan || 'Starter';
    const isSuperAdminUser = state.currentUser?.phone === '08141106560' || (state.currentUser as any)?.phoneNumber === '08141106560' || state.currentUser?.id === '08141106560' || state.currentUser?.email === 'realybash@gmail.com';
    if (!isSuperAdminUser && activeSubscription?.status === 'Active') {
      const existingTerminalsCount = state.posTerminals?.length || 0;
      if (activePlan === 'Starter' && existingTerminalsCount >= 1) {
        showAppNotification('Plan Limitation: Your Starter subscription (₦2,000/mo) only supports 1 physical POS terminal. Please upgrade to Professional (₦5,000/mo) to track up to 3 POS terminals.', 'error');
        return;
      }
      if (activePlan === 'Professional' && existingTerminalsCount >= 3) {
        showAppNotification('Plan Limitation: Your Professional subscription (₦5,000/mo) only supports up to 3 physical POS terminals. Please upgrade to Business (₦10,000/mo) to manage unlimited POS terminals.', 'error');
        return;
      }
    }
  ...
  const handleDeletePosTerminal = async (id: string) => {
    ...
    // 1. Delete permanently from local IndexedDB main cache & pending creation queue
    try {
      await deleteCachedPosTerminal(id);
      await deletePendingPosTerminal(id);
      const current = await getCachedPosTerminals();
  ...
  const handleApproveTransaction = async (tx: Transaction) => {
    ...
    if (syncOwnerId) {
      try {
        ...
        // Increment agent's total Realized Gain in database upon manager approval
        const agentId = syncOwnerId || state.currentUser?.ownerId || state.currentUser?.id;
        if (agentId && updatedProfit > 0) {
          try {
            await setDoc(doc(db, 'users', agentId), {
              totalRealizedGain: increment(updatedProfit),
              realizedGain: increment(updatedProfit),
              updatedAt: serverTimestamp()
            }, { merge: true }).catch(() => {});

            await setDoc(doc(db, 'settings', agentId), {
              totalRealizedGain: increment(updatedProfit),
              realizedGain: increment(updatedProfit),
              updatedAt: serverTimestamp()
            }, { merge: true }).catch(() => {});

            await setDoc(doc(db, 'subscriptions', agentId), {
              totalRealizedGain: increment(updatedProfit),
              realizedGain: increment(updatedProfit),
              updatedAt: serverTimestamp()
            }, { merge: true }).catch(() => {});
          } catch (gainErr) {
            console.warn('[Approval Flow] Error incrementing agent realized gain in DB:', gainErr);
          }
        }
      } catch (err: any) {
  ...
  const handleReverseTransaction = async (tx: Transaction) => {
    if (tx.approvalStatus === 'approved' || tx.approved) {
      showAppNotification('Manager approved debts cannot be reversed once finalized.', 'error');
      return;
    }
  ...
  const handleQuickMarkAsPaid = async (tx: Transaction) => {
    ...
    const newPaymentRecord = {
      ...
      if (state.currentUser?.role === 'Employee') {
        ...
        showAppNotification(`Settlement request of ₦${remainingAmount.toLocaleString()} for ${tx.customerName || 'Customer'} submitted! Pending Manager approval.`, 'info');
        return;
      }
      ...
      showAppNotification(`Successfully marked the debt as FULLY PAID for ${tx.customerName || 'Walk-in Customer'}!`, 'success');
  };
  ...
  const handleSaveInventoryItem = async (itemData: Partial<InventoryItem> & { id?: string }) => {
    // Subscription plan inventory limits checks
    const isSuperAdminUser = state.currentUser?.phone === '08141106560' || (state.currentUser as any)?.phoneNumber === '08141106560' || state.currentUser?.id === '08141106560' || state.currentUser?.email === 'realybash@gmail.com';
    if (!isSuperAdminUser && !itemData.id && activeSubscription?.status === 'Active') {
      const activePlan = activeSubscription?.plan || 'Starter';
      const existingItemsCount = state.inventoryItems?.length || 0;
      if (activePlan === 'Starter' && existingItemsCount >= 100) {
        showAppNotification('Plan Limitation: Your Starter subscription (₦2,000/mo) only supports up to 100 inventory products. Please upgrade to Professional (₦5,000/mo) to track up to 1,000 products.', 'error');
        return;
      }
      if (activePlan === 'Professional' && existingItemsCount >= 1000) {
        showAppNotification('Plan Limitation: Your Professional subscription (₦5,000/mo) only supports up to 1,000 inventory products. Please upgrade to Business (₦10,000/mo) to track unlimited products.', 'error');
        return;
      }
    }

    const now = new Date().toISOString();
    const itemId = itemData.id || `inv_${Date.now()}`;
    const newItem: InventoryItem = {
      id: itemId,
      name: itemData.name || 'Unnamed Product',
      sku: itemData.sku || `SKU-${Math.floor(100000 + Math.random() * 900000)}`,
      barcode: itemData.barcode,
      category: itemData.category || 'POS Accessories',
      costPrice: Number(itemData.costPrice) || 0,
      sellingPrice: Number(itemData.sellingPrice) || 0,
      wholesalePrice: Number(itemData.wholesalePrice) || undefined,
      quantity: Number(itemData.quantity) || 0,
      reorderLevel: Number(itemData.reorderLevel) || 5,
      minimumStock: Number(itemData.minimumStock) || undefined,
      ...
      createdAt: itemData.id ? (state.inventoryItems?.find(i => i.id === itemId)?.createdAt || now) : now,
      updatedAt: now,
      syncOwnerId: syncOwnerId || 'mgr_1'
    };

    dispatch({ type: itemData.id ? 'UPDATE_INVENTORY_ITEM' : 'ADD_INVENTORY_ITEM', payload: newItem });

    if (syncOwnerId) {
      try {
        const cleanData = prepareFirestoreData(newItem, 'inventory_items');
        await setDoc(doc(db, 'inventory_items', itemId), cleanData, { merge: true });
      } catch (err) {
        handleFirestoreError(err, OperationType.WRITE, `inventory_items/${itemId}`);
      }
    }
  };
  ...
  const handleDeleteInventoryItem = async (itemId: string) => {
    ...
    if (syncOwnerId) {
      try {
        ...
        handleFirestoreError(err, OperationType.DELETE, `inventory_items/${itemId}`);
      }
    }
  };

  const handleRestockItem = async (itemId: string, additionalQuantity: number, reason: string) => {
    const item = state.inventoryItems?.find(i => i.id === itemId);
    if (!item) return;
    const newQty = Math.max(0, item.quantity + additionalQuantity);
    const updated = { ...item, quantity: newQty, updatedAt: new Date().toISOString() };
    dispatch({ type: 'UPDATE_INVENTORY_ITEM', payload: updated });

    if (syncOwnerId) {
      try {
        const cleanData = prepareFirestoreData(updated, 'inventory_items');
        await setDoc(doc(db, 'inventory_items', itemId), cleanData, { merge: true });
      } catch (err) {
        handleFirestoreError(err, OperationType.WRITE, `inventory_items/${itemId}`);
      }
  ...
  const handleCompleteSale = async (salePayload: Omit<InventorySale, 'id' | 'createdAt'>): Promise<InventorySale | null> => {
    const saleId = `sale_${Date.now()}`;
    const now = new Date().toISOString();
    const newSale: InventorySale = {
      id: saleId,
      ...salePayload,
      createdAt: now,
      syncOwnerId: syncOwnerId || 'mgr_1'
    ...
    for (const si of salePayload.items) {
      const item = state.inventoryItems?.find(i => i.id === si.itemId);
      if (item) {
        await handleRestockItem(item.id, -si.quantity, `POS Sale ${newSale.saleNumber}`);
      }
    }

    dispatch({ type: 'ADD_INVENTORY_SALE', payload: newSale });

    if (salePayload.paymentMethod === 'Unpaid Debt' && salePayload.customerName) {
      const debtTx: Transaction = {
        id: `tx_debt_${Date.now()}`,
        type: 'Inventory Sale',
        amount: salePayload.totalAmount,
        customerFee: 0,
        terminalFee: 0,
        profit: salePayload.totalProfit,
        status: 'Pending',
        timestamp: now,
        customerName: salePayload.customerName,
        customerPhone: salePayload.customerPhone,
        notes: `Unpaid Inventory Sale #${newSale.saleNumber}`,
        employeeId: state.currentUser.id,
        employeeName: state.currentUser.name,
        terminalId: filteredPosTerminals[0]?.id || 'term_default',
        provider: 'Others'
      };
      await handleAddTransaction(debtTx);
    }

    if (syncOwnerId) {
      try {
        const cleanData = prepareFirestoreData(newSale, 'inventory_sales');
        await setDoc(doc(db, 'inventory_sales', saleId), cleanData);
      } catch (err) {
        handleFirestoreError(err, OperationType.WRITE, `inventory_sales/${saleId}`);
      }
    }
  ...
  const handleSeedInventoryDemo = async () => {
    const demoItems: Partial<InventoryItem>[] = [
      { name: 'OPay Android POS Terminal V3', sku: 'SKU-OPAY-V3', category: 'POS Terminals', costPrice: 45000, sellingPrice: 65000, quantity: 15, reorderLevel: 3, unit: 'pcs', supplier: 'OPay Direct', notes: 'Thermal printer built-in' },
      { name: 'Moniepoint Android POS Terminal', sku: 'SKU-MON-POS', category: 'POS Terminals', costPrice: 48000, sellingPrice: 70000, quantity: 8, reorderLevel: 2, unit: 'pcs', supplier: 'Moniepoint HQ', notes: '4G LTE Enabled' },
      { name: 'POS Thermal Receipt Paper 57mm', sku: 'SKU-ROLL-57', category: 'POS Accessories', costPrice: 350, sellingPrice: 600, quantity: 120, reorderLevel: 20, unit: 'rolls', supplier: 'PaperMill NG', notes: 'Thermal rolls box of 10' },
      { name: 'MTN ₦1,000 Recharge Card Voucher', sku: 'SKU-MTN-1000', category: 'Recharge Cards', costPrice: 970, sellingPrice: 1000, quantity: 50, reorderLevel: 10, unit: 'cards', supplier: 'MTN Nigeria', notes: 'Scratch cards' },
      { name: 'Airtel ₦500 Recharge Card Voucher', sku: 'SKU-AIR-500', category: 'Recharge Cards', costPrice: 485, sellingPrice: 500, quantity: 45, reorderLevel: 10, unit: 'cards', supplier: 'Airtel NG', notes: 'Scratch cards' },
      { name: 'POS Leather Holster Case', sku: 'SKU-CASE-01', category: 'POS Accessories', costPrice: 1500, sellingPrice: 3000, quantity: 25, reorderLevel: 5, unit: 'pcs', supplier: 'Lagos Leather', notes: 'Universal belt holster' }
    ];

    for (const d of demoItems) {
      await handleSaveInventoryItem(d);
    }
  };

  const handleSaveSupplier = async (supplierData: Partial<Supplier> & { id?: string }) => {
    const isNew = !supplierData.id;
    const supplierId = supplierData.id || generateId();
    const now = new Date().toISOString();
    ...
    if (isNew) {
      ...
      dispatch({ type: 'UPDATE_SUPPLIER', payload: newSupplier });
    }

    if (syncOwnerId) {
      try {
        const cleanData = prepareFirestoreData(newSupplier, 'suppliers');
        await setDoc(doc(db, 'suppliers', supplierId), cleanData);
      } catch (err) {
  ...
  const handleRegisterUser = async (newUser: User) => {
    ...
    try {
      // Check employee limits based on active subscription plan
      const isSuperAdminUser = state.currentUser?.phone === '08141106560' || (state.currentUser as any)?.phoneNumber === '08141106560' || state.currentUser?.id === '08141106560' || state.currentUser?.email === 'realybash@gmail.com';
      if (!isSuperAdminUser && newUser.role === 'Employee' && activeSubscription?.status === 'Active') {
        const activePlan = activeSubscription?.plan || 'Starter';
        const existingStaffCount = registeredUsers.filter(u => u.role === 'Employee').length;
        if (activePlan === 'Starter' && existingStaffCount >= 1) {
          throw new Error('Plan Limitation: Your Starter subscription (₦2,000/mo) only supports 1 cashier/staff account. Please upgrade to a Professional plan (₦5,000/mo) to register up to 5 cashiers.');
        }
        if (activePlan === 'Professional' && existingStaffCount >= 5) {
          throw new Error('Plan Limitation: Your Professional subscription (₦5,000/mo) only supports up to 5 cashier/staff accounts. Please upgrade to a Business plan (₦10,000/mo) to manage unlimited cashier logins.');
        }
      }
  ...
  const handleExportCSV = () => {
    if (authorizedTransactions.length === 0) {
      showAppNotification('No record transactions found to export.', 'info');
      return;
    }
  ...
  if (isPremiumLocked) {
    return (
      <div className="min-h-screen bg-neutral-100 flex flex-col font-sans relative pb-10 antialiased selection:bg-red-200">
        
        {/* Custom Header */}
        <header className="bg-white border-b border-neutral-200 py-3.5 px-6 shadow-xs relative z-40">
          <div className="max-w-4xl mx-auto flex items-center justify-between">
            <div className="flex items-center gap-2.5">
              <div className="bg-red-600 text-white p-1.5 rounded-xl font-black text-sm tracking-tight flex items-center justify-center">
                PT
              </div>
              <div>
                <span className="font-extrabold text-neutral-800 tracking-tight block text-sm">POSTrack</span>
                <span className="text-[10px] text-neutral-400 font-bold uppercase tracking-wider block">Oversight Suite</span>
              </div>
            </div>
            
            <button 
              onClick={handleLockTerminal}
              className="bg-neutral-50 hover:bg-neutral-100 text-neutral-600 px-3 py-1.5 rounded-xl text-xs font-black transition cursor-pointer flex items-center gap-1.5 border border-neutral-200/60 shadow-xs"
            >
        ...
        <div className="flex-grow flex items-center justify-center p-4">
          <motion.div 
            ...
            className="bg-white border border-neutral-200 rounded-3xl max-w-lg w-full shadow-xl p-6 md:p-8 space-y-6 text-center overflow-hidden"
          >
            ...
            <div className="space-y-4">
              <div className="relative w-20 h-20 mx-auto">
                ...
                {/* Innermost static background */}
                <div className="absolute inset-2 bg-red-50 border border-red-100 rounded-full flex items-center justify-center text-red-600 shadow-sm z-10">
              ...
              <div className="space-y-2">
                ...
                <div className="inline-flex items-center gap-1.5 px-3 py-1 bg-red-50 text-red-700 rounded-full border border-red-200/50 text-[11px] font-extrabold uppercase tracking-wider font-sans">
            ...
            <div className="text-xs md:text-sm text-neutral-600 space-y-3 leading-relaxed">
              <p className="font-semibold text-neutral-700">
                Your 14-day free trial or paid subscription plan cycle has expired.
              </p>
              <p className="text-neutral-500 text-[12px]">
                To maintain database consistency and continue registering cashiers, daily reconciliations, and transactions, please select a plan and renew your subscription. All of your historical records, reports, settings, and team users are safely saved and fully preserved.
              </p>
            </div>

            {/* Submitted Payment info display card */}
            <div className="bg-neutral-50/80 border border-neutral-200/80 rounded-2xl p-4 text-left space-y-3.5">
              <h4 className="text-[10px] font-black uppercase tracking-widest text-neutral-400 block font-mono border-b border-neutral-200/60 pb-1.5">
                EXPIRED PLAN DETAILS
              </h4>
              
              <div className="grid grid-cols-2 gap-3.5 text-xs">
                <div>
                  <span className="text-neutral-400 font-semibold block text-[10px] uppercase">Plan Name</span>
                  <span className="font-extrabold text-neutral-800">{activeSubscription?.plan || 'Starter Plan'}</span>
                </div>
                <div>
                  <span className="text-neutral-400 font-semibold block text-[10px] uppercase">End Date</span>
                  <span className="font-bold text-neutral-700 font-mono flex items-center gap-1 select-all truncate">
                    {activeSubscription?.subscriptionEndDate ? new Date(activeSubscription.subscriptionEndDate).toLocaleDateString() : 'Expired'}
                  </span>
                </div>
              </div>
            </div>
            ...
            <div className="space-y-2.5 pt-2">
              <button
                onClick={() => {
                  setBillingInitialPlan(activeSubscription?.plan || 'Starter');
                  setIsBillingModalOpen(true);
                }}
                className="w-full bg-[#00B87A] hover:bg-[#009E66] text-white py-3 rounded-2xl font-bold text-sm shadow-sm transition-all transform hover:-translate-y-0.5 active:translate-y-0 cursor-pointer flex items-center justify-center gap-2"
              >
                <Sparkles className="w-4 h-4 text-white" />
                <span>Select a Plan & Renew Now</span>
              </button>

              <button
                onClick={handleRefreshSubscription}
                disabled={isRefreshingSubscription}
                className="w-full bg-white border border-neutral-200 hover:border-neutral-300 text-neutral-700 py-2.5 rounded-2xl font-bold text-xs shadow-xs transition-colors cursor-pointer flex items-center justify-center gap-2 disabled:opacity-65"
              >
                {isRefreshingSubscription ? (
                  <div className="w-4 h-4 border-2 border-neutral-500 border-t-transparent rounded-full animate-spin" />
                ) : (
                  <RefreshCw className="w-3.5 h-3.5 text-[#00B87A]" />
                )}
                <span>Verify Active Subscription / Refresh Status</span>
              </button>

              <button
                onClick={() => {
                  const whatsappUrl = `https://wa.me/2348141106560?text=${encodeURIComponent(`Hello Super Admin, I need help with renewing my subscription on the POSTRACK app. Please assist. Thank you!`)}`;
                  window.open(whatsappUrl, '_blank');
                }}
                className="w-full bg-emerald-50 hover:bg-emerald-100 text-emerald-800 border border-emerald-200/80 py-2.5 rounded-2xl font-bold text-xs transition-colors cursor-pointer flex items-center justify-center gap-2"
              >
        ...
        {/* Modal mounts so they are fully interactive when opened via buttons */}
        <BillingModal
  ...
  if (isPendingReviewAndLocked) {
    return (
      <div className="min-h-screen bg-neutral-100 flex flex-col font-sans relative pb-10 antialiased selection:bg-emerald-200">
        
        {/* Custom Header */}
        <header className="bg-white border-b border-neutral-200 py-3.5 px-6 shadow-xs relative z-40">
          <div className="max-w-4xl mx-auto flex items-center justify-between">
            <div className="flex items-center gap-2.5">
              <div className="bg-[#00B87A] text-white p-1.5 rounded-xl font-black text-sm tracking-tight flex items-center justify-center">
                OP
              </div>
              <div>
                <span className="font-extrabold text-neutral-800 tracking-tight block text-sm">POSTrack</span>
        ...
        <div className="flex-grow flex items-center justify-center p-4">
          ...
          >
            ...
            <div className="space-y-4">
              <div className="relative w-20 h-20 mx-auto">
                {/* Outermost pulsing ring */}
                <div className="absolute inset-0 bg-amber-500/10 rounded-full animate-ping duration-1000" />
                {/* Innermost static background */}
                <div className="absolute inset-2 bg-amber-50 border border-amber-100 rounded-full flex items-center justify-center text-amber-600 shadow-sm z-10">
                  <Clock className="w-9 h-9 animate-spin" style={{ animationDuration: '8s' }} />
                </div>
              </div>
              ...
              <div className="space-y-2">
                ...
                <div className="inline-flex items-center gap-1.5 px-3 py-1 bg-amber-50 text-amber-700 rounded-full border border-amber-200/50 text-[11px] font-extrabold uppercase tracking-wider font-sans">
            ...
            <div className="text-xs md:text-sm text-neutral-600 space-y-3 leading-relaxed">
              <p className="font-semibold text-neutral-700">
                Your payment receipt has been successfully uploaded and is undergoing review by the Super Admin manager.
              </p>
              <p className="text-neutral-500 text-[12px]">
                To maintain transaction integrity, cash registers and management functions are temporarily locked during this review window. Once confirmed and approved, your account will instantly start working and functioning correctly.
              </p>
            </div>
            ...
            <div className="bg-neutral-50/80 border border-neutral-200/80 rounded-2xl p-4 text-left space-y-3.5">
              ...
              <div className="grid grid-cols-2 gap-3.5 text-xs">
                <div>
                  <span className="text-neutral-400 font-semibold block text-[10px] uppercase">Plan Requested</span>
                  <span className="font-extrabold text-neutral-800">{activeSubscription?.plan || 'Standard Plan'}</span>
                </div>
                <div>
                  <span className="text-neutral-400 font-semibold block text-[10px] uppercase">Reference ID</span>
                  <span className="font-bold text-neutral-700 font-mono flex items-center gap-1 select-all truncate">
                    {activeSubscription?.lastPaymentReference || 'N/A'}
                  </span>
                </div>
              </div>

              {/* Uploaded Receipt Preview */}
              {activeSubscription?.lastReceiptUrl && (
                <div className="pt-2">
                  <span className="text-neutral-400 font-semibold block text-[10px] uppercase mb-1.5">Your Uploaded Receipt Proof</span>
                  <div className="flex items-center gap-3 bg-white p-2 rounded-xl border border-neutral-200">
                    <img 
                      src={activeSubscription.lastReceiptUrl} 
                      alt="Payment Receipt Thumbnail" 
                      className="w-12 h-12 rounded-lg object-cover border border-neutral-100 shrink-0 cursor-pointer hover:opacity-80"
                      onClick={() => setIsReceiptZoomed(true)}
                    />
                    <div className="flex-1 min-w-0">
                      <span className="text-[11px] font-bold text-neutral-700 block truncate">receipt_proof_document.png</span>
                      <button 
                        type="button" 
                        onClick={() => setIsReceiptZoomed(true)}
                        className="text-[#00B87A] hover:underline font-extrabold text-[10px] tracking-tight text-left block"
                      >
                        View Full Receipt Proof
                      </button>
                    </div>
                  </div>
                </div>
            ...
            <div className="space-y-2.5 pt-2">
              <button
                onClick={handleRefreshSubscription}
                disabled={isRefreshingSubscription}
                className="w-full bg-[#00B87A] hover:bg-[#009E66] text-white py-3 rounded-2xl font-bold text-sm shadow-sm transition-all transform hover:-translate-y-0.5 active:translate-y-0 cursor-pointer flex items-center justify-center gap-2 disabled:opacity-65 font-sans"
              >
                {isRefreshingSubscription ? (
              ...
              <button
                ...
                className="w-full bg-white border border-neutral-200 hover:border-neutral-300 text-neutral-700 py-2.5 rounded-2xl font-bold text-xs shadow-xs transition-colors cursor-pointer flex items-center justify-center gap-2"
              >
                <Upload className="w-3.5 h-3.5 text-[#00B87A]" />
                <span>Change Plan or Upload Different Receipt</span>
              </button>

              <button
                onClick={() => {
                  const whatsappUrl = `https://wa.me/2348141106560?text=${encodeURIComponent(`Hello Super Admin, I have paid and uploaded my subscription payment receipt for reference ${activeSubscription?.lastPaymentReference || ''} on the POSTRACK app. Please review and approve my subscription. Thank you!`)}`;
                  window.open(whatsappUrl, '_blank');
                }}
                className="w-full bg-emerald-50 hover:bg-emerald-100 text-emerald-800 border border-emerald-200/80 py-2.5 rounded-2xl font-bold text-xs transition-colors cursor-pointer flex items-center justify-center gap-2"
              >
        ...
        {/* Zoom Lightbox Popup */}
        <AnimatePresence>
          {isReceiptZoomed && activeSubscription?.lastReceiptUrl && (
            <div className="fixed inset-0 bg-black/85 backdrop-blur-xs flex items-center justify-center p-4 z-[210]">
              <motion.div 
                initial={{ opacity: 0, scale: 0.95 }}
                animate={{ opacity: 1, scale: 1 }}
                exit={{ opacity: 0, scale: 0.95 }}
                className="bg-white border border-neutral-200 max-w-2xl w-full rounded-3xl overflow-hidden shadow-2xl flex flex-col relative animate-in zoom-in-95 duration-200"
              >
                <div className="px-5 py-3 border-b border-neutral-200 flex justify-between items-center bg-neutral-50 shrink-0">
                  <span className="text-xs font-extrabold text-neutral-700">Receipt Proof Zoom</span>
                  <button 
                    onClick={() => setIsReceiptZoomed(false)}
                    className="p-1 hover:bg-neutral-200 rounded-full transition text-neutral-500 cursor-pointer"
                  >
                    <X className="w-5 h-5" />
                  </button>
                </div>
                <div className="p-4 overflow-y-auto max-h-[75vh] flex justify-center bg-neutral-900">
                  <img 
                    src={activeSubscription.lastReceiptUrl} 
                    alt="Receipt Proof" 
                    className="max-h-[70vh] max-w-full object-contain shadow-md rounded-lg"
                  />
                </div>
                <div className="p-4 border-t border-neutral-200 bg-neutral-50 text-center text-[11px] text-neutral-500 font-medium shrink-0">
                  Showing original receipt uploaded with payment reference: <strong>{activeSubscription?.lastPaymentReference}</strong>
                </div>
              </motion.div>
            </div>
          )}
    ...
    <div className="min-h-screen bg-neutral-100 text-neutral-850 flex flex-col font-sans relative pb-20 antialiased selection:bg-emerald-200">
      ...
      {unpaidCount > 0 && (
        <div 
          ...
          onMouseEnter={() => setIsHoveringUnpaidBanner(true)}
          onMouseLeave={() => setIsHoveringUnpaidBanner(false)}
          className="bg-[#FF6C00] text-white shadow-md z-40 sticky top-0 border-b border-orange-700/30 select-none cursor-pointer hover:brightness-[1.02] transition-all duration-150 overflow-hidden"
        >
          <AnimatePresence mode="wait">
            {(() => {
              const currentTx = unpaidTransactions[activeUnpaidIndex];
              const isChargeSettled = currentTx && (currentTx.chargesStatus === 'Paid' || currentTx.chargesStatus === 'Settled' || currentTx.chargesStatus === 'Waived' || currentTx.chargesStatus === 'Waive') && !currentTx.pendingSettlement;
              
              if (!currentTx || isChargeSettled) return null;
              
              const remainingAmount = currentTx.unpaidFeeAmount !== undefined ? currentTx.unpaidFeeAmount : (currentTx.customerFee || 0);
              const debtorName = (currentTx.customerName || 'Walk-in Customer').toUpperCase();
              
              const txDate = currentTx.timestamp ? new Date(currentTx.timestamp) : new Date();
              const isValidDate = !isNaN(txDate.getTime());
              const formattedDate = isValidDate ? txDate.toLocaleDateString('en-US', { month: 'short', day: '2-digit', year: 'numeric' }) : 'Today';
              const formattedTime = isValidDate ? txDate.toLocaleTimeString('en-US', { hour: '2-digit', minute: '2-digit', hour12: true }) : '';
              
              const receiptCode = currentTx.receiptNumber ? `#${currentTx.receiptNumber}` : currentTx.id ? `#${String(currentTx.id).slice(0, 6).toUpperCase()}` : '#REF';
              const cashierName = (currentTx.cashierName || state.currentUser?.name || 'Cashier').toUpperCase();
              const branchName = state.settings?.businessName || 'My Agency';

              return (
                <motion.div
                  key={currentTx.id || activeUnpaidIndex}
                  initial={{ opacity: 0, x: 20 }}
                  animate={{ opacity: 1, x: 0 }}
                  exit={{ opacity: 0, x: -20 }}
                  transition={{ duration: 0.2, ease: "easeInOut" }}
                  className="flex flex-row items-stretch justify-between min-w-0"
                >
                  {/* Left Dark Box: CHARGE / Count / Fee */}
                  <div className="bg-[#DF5A00] px-4 py-3.5 flex flex-col items-center justify-center text-center shrink-0 border-r border-orange-600/30 min-w-[85px] sm:min-w-[100px]">
                    <div className="w-7 h-7 rounded-xl border border-white/30 flex items-center justify-center mb-1.5 bg-white/10 shadow-xs">
                      <AlertTriangle className="w-4 h-4 text-white" />
                    </div>
                    <span className="text-[9px] font-black uppercase tracking-wider text-amber-100 opacity-90 leading-none">
                      CHARGE
                    </span>
                    <span className="text-xl sm:text-2xl font-black font-mono leading-tight my-0.5 text-white">
                      {activeUnpaidIndex + 1}
                    </span>
                    <span className="text-[10px] font-mono text-amber-100/95 font-bold">
                      {formatNaira(remainingAmount)}
                    </span>
                  </div>

                  {/* Middle Section: Detailed Card Fields */}
                  <div className="flex-1 p-3.5 sm:p-4 text-xs leading-relaxed flex flex-col justify-center min-w-0 space-y-1">
                    <div className="flex items-center justify-between gap-2">
                      <div className="flex items-center gap-1.5">
                        ...
                        {unpaidCount > 1 && (
                          <span className="text-[9px] font-mono font-bold text-amber-100/90 bg-black/20 px-1.5 py-0.5 rounded ml-1">
                            ({activeUnpaidIndex + 1}/{unpaidCount})
                          </span>
                        )}
                      </div>

                      {/* Manual Carousel Rotation Controls when multiple unpaid charges exist */}
                      {unpaidCount > 1 && (
                        <div 
                          className="flex items-center gap-1 bg-black/25 rounded-lg px-1.5 py-0.5 border border-white/20 shrink-0" 
                          onClick={(e) => e.stopPropagation()}
                        >
                          <button
                            type="button"
                            onClick={() => setActiveUnpaidIndex((prev) => (prev - 1 + unpaidCount) % unpaidCount)}
                            className="p-1 hover:bg-white/20 active:scale-90 rounded text-amber-100 hover:text-white transition cursor-pointer"
                            title="Previous Charge"
                          >
                            <ChevronLeft className="w-3.5 h-3.5" />
                          </button>
                          <span className="text-[10px] font-mono font-black px-1 text-amber-100">
                            {activeUnpaidIndex + 1} / {unpaidCount}
                          </span>
                          <button
                            type="button"
                            onClick={() => setActiveUnpaidIndex((prev) => (prev + 1) % unpaidCount)}
                            className="p-1 hover:bg-white/20 active:scale-90 rounded text-amber-100 hover:text-white transition cursor-pointer"
                            title="Next Charge"
                          >
                    ...
                    <div className="flex flex-col space-y-0.5 text-white/95 text-[11px] sm:text-xs">
                      <div>
                        <span className="text-amber-100/80">Customer: </span>
                        <strong className="font-extrabold text-white">{debtorName}</strong>
                      </div>
                      <div>
                        <span className="text-amber-100/80">Transaction: </span>
                        <strong className="font-bold text-white">{currentTx.type}</strong>
                      </div>
                      <div>
                        <span className="text-amber-100/80">Amount: </span>
                        <strong className="font-mono font-black text-white">{formatNaira(currentTx.amount)}</strong>
                      </div>
                      <div className="flex items-center gap-1.5">
                        <span className="text-amber-100/80">Outstanding: </span>
                        <span className="bg-black/35 text-white font-mono font-bold px-2 py-0.5 rounded text-[10px] tracking-wide">
                          {formatNaira(remainingAmount)}
                        </span>
                      </div>
                      <div>
                        <span className="text-amber-100/80">Date: </span>
                        <strong className="font-bold text-white">{formattedDate}</strong>
                      </div>
                      <div>
                        <span className="text-amber-100/80">Time: </span>
                        <strong className="font-bold text-white">{formattedTime}</strong>
                      </div>
                      <div>
                        <span className="text-amber-100/80">Cashier: </span>
                        <strong className="font-bold text-white">{cashierName}</strong>
                      </div>
                      <div>
                        <span className="text-amber-100/80">Branch: </span>
                        <strong className="font-bold text-white">{branchName}</strong>
                      </div>
                      <div>
                        <span className="text-amber-100/80">Receipt: </span>
                        <strong className="font-mono font-bold text-white">{receiptCode}</strong>
                      </div>
                    </div>
                  </div>

                  {/* Right Action Button: Settle */}
                  <div className="px-3 py-3 sm:px-5 flex items-center justify-end shrink-0 bg-[#FF6C00]">
                    <button
                      type="button"
                      onClick={(e) => {
                        e.stopPropagation();
                      ...
                      className="bg-[#00B87A] hover:bg-[#00A36C] active:scale-95 text-white px-4 py-2 rounded-xl text-xs sm:text-sm font-black flex items-center gap-1.5 shadow-md transition-all duration-150 cursor-pointer"
                    >
      ...
      <header className="border-b border-neutral-200 bg-white sticky top-0 z-30 shadow-sm">
        <div className="max-w-4xl mx-auto px-4 py-3 flex flex-col sm:flex-row items-center justify-between gap-3 sm:gap-4">
          ...
          <div className="flex flex-wrap items-center justify-center sm:justify-end gap-3 w-full sm:w-auto mt-1 sm:mt-0">
            ...
            <button 
              onClick={() => showAppNotification("Gateway connection is extremely stable. High velocity is active.", "success")}
      ...
      <main className="max-w-4xl mx-auto px-4 py-5 flex-grow space-y-6 w-full">
        ...
        {activeUser.role === 'Manager' && (
          <div className="bg-white border border-neutral-200 rounded-[32px] p-6 shadow-xs relative overflow-hidden transition-all duration-150">
            ...
            {/* DYNAMIC DEBT / UNPAID CHARGES BANNER - ONLY SHOWS IF DEBT OR UNPAID CHARGES EXIST ON ACCOUNT */}
            {unpaidCount > 0 && (
              <div 
                onClick={() => setDashboardTab('unpaid')}
                className="bg-amber-50 text-amber-900 rounded-xl p-4 shadow-sm flex items-center justify-between gap-3 cursor-pointer hover:bg-amber-100 transition border border-amber-200 mb-5"
              >
                  ...
                  <div className="min-w-0">
                    <div className="flex items-center gap-2">
                      <span className="text-xs font-bold text-amber-800">
                        {unpaidCount} {unpaidCount === 1 ? 'Unpaid Charge' : 'Unpaid Charges'}
                      </span>
                    </div>
                    <p className="text-sm text-amber-900/70 font-medium mt-0.5 truncate">
                      Outstanding balance: <strong className="font-semibold text-amber-950">{formatNaira(totalUnpaidDebtAmount)}</strong>
                    </p>
                  </div>
                </div>
                <div className="flex items-center gap-1 text-xs font-semibold text-amber-900 shrink-0 bg-white px-3 py-1.5 rounded-lg border border-amber-200 shadow-sm">
                  <span>Review</span>
                  <span>→</span>
                </div>
              </div>
        ...
        {activeUser.role === 'Employee' && (
          <div className="bg-gradient-to-br from-emerald-950 via-neutral-900 to-neutral-950 text-white border border-neutral-800 rounded-[32px] p-6 shadow-xl relative overflow-hidden transition-all duration-150">
            ...
            {unpaidCount > 0 && (
              <div 
                onClick={() => setDashboardTab('unpaid')}
                className="bg-gradient-to-r from-red-600 via-rose-600 to-amber-600 text-white rounded-2xl p-3.5 shadow-sm flex items-center justify-between gap-3 cursor-pointer hover:brightness-105 transition border border-red-500/30 mb-5 animate-pulse relative z-20"
              >
                <div className="flex items-center gap-3 min-w-0">
                  ...
                  <div className="min-w-0">
                    <div className="flex items-center gap-2">
                      <span className="text-[10px] font-mono font-black uppercase tracking-wider text-amber-200 bg-red-950/40 px-2 py-0.5 rounded-md">
                        ⚠️ ACCOUNT DEBT ALERT ({unpaidCount} {unpaidCount === 1 ? 'UNPAID CHARGE' : 'UNPAID CHARGES'})
                      </span>
                    </div>
                    <p className="text-xs text-white/95 font-semibold mt-1 truncate">
                      Outstanding Unpaid Balance: <strong className="font-mono font-black text-amber-200">{formatNaira(totalUnpaidDebtAmount)}</strong>
                    </p>
                  </div>
                </div>
                <div className="flex items-center gap-1 text-[10px] font-black uppercase font-mono tracking-wider shrink-0 bg-white/20 hover:bg-white/30 px-3 py-1.5 rounded-xl border border-white/30">
        ...
        <div className="bg-neutral-200/50 border border-neutral-300/40 p-3 rounded-2xl flex items-center justify-between text-neutral-700 text-xs gap-3">
          ...
          <button 
            type="button"
            onClick={() => showAppNotification("Security verification guide: 1. Confirm transaction slips. 2. Verify alert directly inside this app. 3. Ensure baseline rates align.", "info")}
        ...
        <div className={`bg-white border border-neutral-200 p-1.5 rounded-3xl shadow-sm grid gap-1 select-none ${
          ...
          >
            ...
            <span className="text-[10px] font-black tracking-tight leading-none">Inventory</span>
          </button>
          <button
            type="button"
        ...
        {dashboardTab === 'pos' && (
          <>
            ...
            >
              ...
              <span className="text-[11px] font-bold text-neutral-700 leading-tight">Inventory</span>
            </button>
        ...
        {dashboardTab === 'settings' && (
          <ErrorBoundary sectionTitle="Commissions Settings">
        ...
        {dashboardTab === 'expenses' && (
          <ErrorBoundary sectionTitle="Expense Tracking">
        ...
        {dashboardTab === 'terminals' && (
          <ErrorBoundary sectionTitle="Registered POS Terminals">
              ...
              <form
                onSubmit={(e) => {
                  ...
                  if (!newTerminalName.trim()) {
                    showAppNotification('Name of the POS is required.', 'error');
                    return;
                  }
                  if (!newTerminalAccountNo.trim()) {
                    showAppNotification('POS Account number is required.', 'error');
                    return;
                  }
                  if (!newTerminalCashierName.trim()) {
                    showAppNotification('Name of the cashier is required.', 'error');
                    return;
                  }
                  if (!newTerminalArea.trim()) {
                    showAppNotification('Area of working is required.', 'error');
                    return;
                  }
        ...
        {dashboardTab === 'inventory' && (
          <ErrorBoundary sectionTitle="Inventory & POS Sales">
            <InventoryManager
              items={state.inventoryItems || []}
              sales={state.inventorySales || []}
              ...
              onSeedDemo={handleSeedInventoryDemo}
        ...
        {dashboardTab === 'unpaid' && (
          <ErrorBoundary sectionTitle="Unpaid Charges">
            <UnpaidChargesLedger
        ...
        {dashboardTab === 'referrals' && state.currentUser.role === 'Manager' && (
          <ErrorBoundary sectionTitle="Referrals">
            <ReferralsTab 
              currentUser={state.currentUser}
              showAppNotification={showAppNotification}
        ...
        {dashboardTab === 'settings' && isSuperAdmin && (
          <ErrorBoundary sectionTitle="Configuration & Secondary Settings">
            <>
          {state.currentUser.role === 'Manager' && (
            <div className="bg-white border border-neutral-200 p-4 rounded-3xl shadow-sm flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
              <div className="space-y-0.5">
        ...
        {dashboardTab === 'reports' && (
          <ErrorBoundary sectionTitle="Reports Charts & Analytics">
        ...
        <div ref={historySectionRef}>
          <ErrorBoundary sectionTitle="Transaction Ledger List">
            <TransactionList
              currentUser={state.currentUser}
              transactions={authorizedHistoryTransactions}
              historyFilter={state.historyFilter}
        ...
        {dashboardTab === 'audit' && isSuperAdmin && (
          <ErrorBoundary sectionTitle="Admin Pricing Audit">
        ...
        {dashboardTab === 'pricing' && (
          <ErrorBoundary sectionTitle="Pricing Rule Manager">
            <PricingRuleManager 
              currentUser={activeUser} 
              settings={state.settings} 
        ...
        {dashboardTab === 'payment-audit' && isSaaSAdmin && (
          <ErrorBoundary sectionTitle="Audit Payment History">
      ...
      <footer className="fixed bottom-0 left-0 right-0 z-45 bg-white border-t border-neutral-200 py-2 shadow-lg">
        <div className="max-w-md mx-auto px-4 flex items-center justify-between text-center select-none text-[10px] text-neutral-400 font-bold">
          ...
          <button 
            type="button"
            onClick={() => showAppNotification("Already viewing OPay Manager Home screen", "info")}
                  ...
                  <div className="grid grid-cols-2 gap-2.5 pt-2">
                    ...
                    <button
                      ...
                      onClick={async () => {
                        ...
                        // --- Paystack Online Settlement Flow ---
                        if (carouselSettleMethod === 'Paystack') {
                          setIsPaystackProcessing(true);
                          try {
                            // 1. Load Paystack Inline JS dynamically for offline robustness
                            ...
                            const reference = 'ps_ref_' + generateId() + '_' + Math.random().toString(36).substring(2, 7);
                            const customerEmail = state.currentUser?.email || 'customer@dangodalpostracker.com';
                            ...
                            const paystackPop = (window as any).PaystackPop.setup({
                              ...
                              callback: function (response: any) {
                                (async () => {
                                  showAppNotification("Verifying payment...", "info");
                                  try {
                                    ...
                                    if (verifyData.status && verifyData.data?.status === "success") {
                                      // Save the successful verified payment records
                                      const verifiedRecord = {
                                        ...newPaymentRecord,
                                        note: `${carouselSettleNote.trim() || 'Carousel Settle'} (Paid via Paystack, Ref: ${response.reference})`
                                      };
                                      
                                      const updatedVerifiedPayments = [...(carouselSettlingTx.chargePayments || []), verifiedRecord];
                                      ...
                                      if (state.currentUser?.role === 'Employee') {
                                        const updatedTx: Transaction = {
                                          ...
                                          pendingSettlement: {
                                            ...
                                            proposedPaymentRecord: verifiedRecord
                                          }
                                        };
                                        await handleUpdateTransaction(updatedTx);
                                        showAppNotification(`Secure payment verified! Settlement request of ₦${currentPayment.toLocaleString()} submitted for approval.`, 'success');
                                      } else {
                                        const updatedTx: Transaction = {
                                          ...carouselSettlingTx,
                                          customerFee: finalCustomerFee,
                                          ...
                                          chargePayments: updatedVerifiedPayments
                                        };
                                        await handleUpdateTransaction(updatedTx);
                                        showAppNotification(`Successfully processed secure Paystack payment of ${formatNaira(currentPayment)}!`, 'success');
                                      }
                            ...
                            showAppNotification("Error launching secure payment popup.", "error");
                            setIsPaystackProcessing(false);
                          }
                          return;
                        }
                        ...
                        if (state.currentUser?.role === 'Employee') {
                          ...
                          await handleUpdateTransaction(updatedTx);
                          showAppNotification(`Settlement request of ₦${currentPayment.toLocaleString()} for ${carouselSettlingTx.customerName || 'Customer'} submitted! Pending Manager approval.`, 'info');
                          setCarouselSettlingTx(null);
                          return;
                        }
                        ...
                        showAppNotification(`Successfully recorded payment of ${formatNaira(currentPayment)} for ${carouselSettlingTx.customerName || 'Customer'}!`, 'success');
                        setCarouselSettlingTx(null);
                      }}
                      className="py-2.5 px-4 bg-[#00B87A] hover:bg-emerald-600 text-white font-black rounded-xl text-xs font-mono uppercase shadow-md active:scale-95 cursor-pointer flex items-center justify-center disabled:opacity-50 disabled:cursor-not-allowed"
                      disabled={isPaystackProcessing || currentPayment <= 0}
                    >
      ...
      <AnimatePresence>
        {isPwaInstallModalOpen && (
          <div className="fixed inset-0 bg-neutral-900/60 backdrop-blur-xs z-55 flex items-center justify-center p-4">
            <motion.div
              initial={{ opacity: 0, scale: 0.95, y: 10 }}
              animate={{ opacity: 1, scale: 1, y: 0 }}
              exit={{ opacity: 0, scale: 0.95, y: 10 }}
              className="bg-white border border-neutral-200/80 rounded-3xl max-w-sm w-full shadow-2xl p-6 relative overflow-hidden space-y-5"
            >
              <button
                onClick={handleDismissPwaModal}
                className="absolute top-4 right-4 bg-neutral-100 hover:bg-neutral-200 text-neutral-500 hover:text-neutral-700 p-2 rounded-full transition cursor-pointer"
              >
                <X className="w-4 h-4" />
              </button>

              <div className="text-center space-y-1.5 pt-2">
                <div className="w-12 h-12 bg-emerald-50 rounded-2xl flex items-center justify-center mx-auto border border-emerald-100 text-[#00B87A] shadow-sm">
                  <Smartphone className="w-6 h-6 animate-pulse" />
                </div>
                <h3 className="text-base font-black text-neutral-850 tracking-tight uppercase font-mono">
                  Install Mobile App
                </h3>
                <p className="text-[11px] text-neutral-400 font-medium max-w-[250px] mx-auto leading-relaxed">
                  Install POSTrack directly on your phone to run it in fullscreen standalone app mode!
                </p>
              </div>

              <button
                onClick={triggerDirectInstall}
                className="w-full bg-[#00B87A] hover:bg-[#009E66] text-white py-2.5 rounded-xl font-bold text-xs transition cursor-pointer flex items-center justify-center gap-2 border shadow-sm"
              >
                <Download className="w-4 h-4" />
                <span>Install Directly Now</span>
              </button>
            </motion.div>
          </div>
        )}
      ...
      {/* DIRECT PWA INSTALL FLOATING ACTION BUTTON */}
      {!isStandalone && isInstallable && (
        <motion.button
          ...
          whileTap={{ scale: 0.95 }}
          onClick={triggerDirectInstall}
          className="fixed bottom-[150px] right-4 sm:bottom-[166px] sm:right-6 z-50 flex items-center justify-center gap-2 px-3.5 py-3.5 bg-neutral-900 hover:bg-neutral-800 text-white rounded-full shadow-2xl border border-neutral-700/50 cursor-pointer active:scale-95 transition-all"
          title="Install POSTRACKER on This Device"
        >
          <Download className="w-5 h-5 text-[#00B87A] animate-bounce" />
          <span className="text-[11px] font-black uppercase tracking-wider font-mono pr-1 hidden md:inline">Install POSTRACKER</span>
        </motion.button>
      )}
      ...
      {approvalToast && (
        <div className="fixed bottom-6 right-6 z-50 animate-in slide-in-from-bottom-5 duration-300">
          <div className="bg-neutral-900/95 backdrop-blur-md border-2 border-emerald-500/80 text-white rounded-2xl p-4 shadow-2xl flex items-center gap-4 max-w-md">
            ...
            <div className="bg-emerald-500/20 text-emerald-300 font-mono font-black text-[10px] px-2.5 py-1.5 rounded-xl border border-emerald-500/30 flex items-center gap-1 shrink-0">

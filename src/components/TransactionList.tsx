              ...
              <AnimatePresence mode="popLayout">
                {paginatedList.map((tx) => {
                  ...
                  const amountTextSize = 
                    cardSize === 'small' 
                      ? 'text-[14px] sm:text-[15px] font-black font-mono tracking-tighter' 
                      : cardSize === 'medium'
                      ? 'text-[16px] sm:text-[17px] font-black font-mono tracking-tighter'
                      : 'text-[18px] sm:text-[19.5px] font-black font-mono tracking-tighter';
                  ...
                  return (
                    ...
                    >
                      ...
                      <div className={`flex justify-between items-start ${gapSpacing}`}>
                        <div className={`flex ${gapSpacing} min-w-0 items-start`}>
                          ...
                          <div className="min-w-0 space-y-1 flex-1">
                            {/* Header Row: Title + Providers + Status Badges */}
                            <div className="flex items-center gap-1.5 flex-wrap">
                              <h4 className={`${titleTextSize} text-neutral-900 tracking-tight font-black uppercase`}>
                                {easyCategoryTitle}
                              </h4>
                              <span className="text-neutral-200 text-[9px] select-none">•</span>
                              <div className="flex items-center gap-1 flex-wrap">
                                ...
                                {isUnpaidDebt && (
                                  <span className={`${cardSize === 'small' ? 'text-[7px] px-1.5' : 'text-[8px] px-2'} font-black py-0.2 rounded-full bg-rose-600 text-white border border-rose-700 uppercase tracking-wider flex items-center gap-1 shadow-xs animate-pulse`}>
                                    ...
                                    DEBT ({tx.chargesStatus === 'PartiallyPaid' ? 'PARTIALLY UNPAID' : 'UNPAID CHARGES'})
                                  </span>
                                )}
                            ...
                            <div className="flex flex-wrap items-center gap-1.5 pt-0.5">
                              <div className={`${cardSize === 'small' ? 'px-1.5 py-0.2 text-[8px]' : 'px-2 py-0.5 text-[9px]'} rounded-md font-black flex items-center gap-1 shadow-xs ${
                                ...
                                {isUnpaidDebt && <span className="text-[7.5px] uppercase tracking-wider text-rose-700 bg-rose-100 px-1 rounded font-black">(DEBT)</span>}
                              </div>
                        ...
                        <div className={`flex flex-col items-end ${cardSize === 'small' ? 'gap-1' : 'gap-1.5'} text-right shrink-0 pt-0.5`}>
                          <div className={`${amountTextSize} leading-none text-neutral-900`}>
                            {formatNaira(tx.amount)}
                          </div>
                          
                          <div className={`flex items-center flex-wrap justify-end ${cardSize === 'small' ? 'gap-1' : 'gap-1.5'}`}>
                             ...
                             <div className={`${cardSize === 'small' ? 'p-0.5 rounded-md' : 'p-0.8 rounded-lg'} transition-all duration-300 ${isExpanded ? 'bg-neutral-900 text-white shadow-lg' : 'bg-neutral-50 text-neutral-400'}`}>
                                {isExpanded ? <ChevronUp className={cardSize === 'small' ? 'w-3 h-3' : 'w-3.5 h-3.5'} /> : <ChevronDown className={cardSize === 'small' ? 'w-3 h-3' : 'w-3.5 h-3.5'} />}
                             </div>
                          </div>
                        </div>
                      </div>

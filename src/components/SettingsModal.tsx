        ...
        <div className="flex flex-col md:flex-row flex-grow overflow-hidden">
          ...
          <div className="flex-grow p-5 md:p-6 overflow-y-auto space-y-5">
            ...
            {activeTab === 'receipt' && (() => {
              ...
              const isSuperAdminUser = isSuperAdmin || activeSubscription === null; 
              const isTrial = activeSubscription?.status === 'Trial';
              const hasBrandingAccess = isSuperAdminUser || isTrial || activePlan === 'Business' || activePlan === 'Enterprise';

              if (!hasBrandingAccess) {
                return (
                  <div className="space-y-5 animate-fade-in py-8 px-5 text-center bg-neutral-50/50 border border-neutral-200/60 rounded-3xl">
                    <div className="w-12 h-12 bg-amber-50 text-amber-600 rounded-full flex items-center justify-center mx-auto border border-amber-100 shadow-xs">
                      <Lock className="w-5 h-5" />
                    </div>
                    <div className="space-y-1.5">
                      <h4 className="text-xs font-black uppercase text-neutral-850 tracking-wider">Custom Branding Locked</h4>
                      <p className="text-[11px] text-neutral-500 max-w-sm mx-auto leading-relaxed font-medium">
                        Custom receipt business headers, office address, contact phone, and custom footer warm greetings are exclusive to the <strong>Business Plan (₦10,000/mo)</strong> or higher.
                      </p>
                    </div>
                    <div className="text-[10px] text-neutral-450 bg-neutral-100/60 px-3 py-1 inline-block rounded-full font-bold">
                      Current Plan Tier: <span className="text-[#00B87A]">{activePlan}</span>
                    </div>
                  </div>
                );
              }

...
export function ManagerAggregatedStats({ transactions, registeredUsers }: ManagerAggregatedStatsProps) {
  const isToday = (tx: Transaction) => {
    const d = getTxEffectiveDate(tx);
  ...
  const cashierStats = useMemo(() => {
    // We only care about transactions performed today or approved today
    const todayTxs = transactions.filter(tx => isToday(tx));

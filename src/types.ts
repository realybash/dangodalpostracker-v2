...
export type TransactionType = 'Deposit' | 'Withdrawal' | 'Transfer' | 'Cash In' | 'Cash Out' | 'Cash Out (Transfer)' | 'Airtime' | 'Data' | 'Bills' | 'Inventory Sale' | 'Goods Sale';
...
export interface InventorySale {
  ...
  totalCost: number;
  totalProfit: number;
  paymentMethod: 'Cash' | 'POS Card' | 'Bank Transfer' | 'Unpaid Debt';
  customerName?: string;
...
export interface Transaction {
  ...
  status?: 'Success' | 'Pending' | 'Failed' | 'approved' | 'Approved' | string;
  ownerId?: string;
...
export type SecurityEventType = 'Login' | 'Logout' | 'Assignment' | 'Sale' | 'Product Added' | 'Product Deleted' | 'Unauthorized' | 'Denied' | 'Other';
